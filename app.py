...
def run_pitcher_offense_platoon_test(master_df,min_prior_games=12,min_pitcher_starts=3,min_platoon_pa=80.0):
    ...
    if not v15.get("Error") and isinstance(v15.get("bets"),pd.DataFrame) and not v15["bets"].empty:
        ...
        for name,g in b.groupby("Month"): seg.append({"Segment_Group":"Month","Segment":name,**summarize_sim_bets(g)})
...
def run_pitcher_offense_lineup_test(master_df,min_prior_games=12,min_pitcher_starts=3,min_platoon_pa=80.0,min_hitter_pa=20.0):
    ...
    if not v16.get("Error") and isinstance(v16.get("bets"),pd.DataFrame) and not v16["bets"].empty:
        ...
        for name,g in b.groupby("Month"): seg.append({"Segment_Group":"Month","Segment":name,**summarize_sim_bets(g)})
        for name,g in [("Favorites",b[b["Odds"]<0]),("Underdogs",b[b["Odds"]>0])]:
            seg.append({"Segment_Group":"Side","Segment":name,**summarize_sim_bets(g)})

        # Frozen 2025 price buckets. These are diagnostics only; they do not alter thresholds.
        price_specs = [
            ("Favorite -200 or shorter", b["Odds"] <= -200),
            ("Favorite -150 to -199", (b["Odds"] <= -150) & (b["Odds"] > -200)),
            ("Favorite -110 to -149", (b["Odds"] < 0) & (b["Odds"] > -150)),
            ("Dog +100 to +149", (b["Odds"] >= 100) & (b["Odds"] < 150)),
            ("Dog +150 to +199", (b["Odds"] >= 150) & (b["Odds"] < 200)),
            ("Dog +200 to +249", (b["Odds"] >= 200) & (b["Odds"] < 250)),
            ("Dog +250 to +299", (b["Odds"] >= 250) & (b["Odds"] < 300)),
        ]
        for name, mask in price_specs:
            g=b[mask]
            seg.append({"Segment_Group":"Price bucket","Segment":name,**summarize_sim_bets(g)})

        edge_specs = [
            ("Edge 2.5–4.9%", (b["Edge"] >= .025) & (b["Edge"] < .05)),
            ("Edge 5.0–7.4%", (b["Edge"] >= .05) & (b["Edge"] < .075)),
            ("Edge 7.5–9.9%", (b["Edge"] >= .075) & (b["Edge"] < .10)),
            ("Edge 10.0–14.9%", (b["Edge"] >= .10) & (b["Edge"] < .15)),
            ("Edge 15%+", b["Edge"] >= .15),
        ]
        for name, mask in edge_specs:
            g=b[mask]
            seg.append({"Segment_Group":"Edge bucket","Segment":name,**summarize_sim_bets(g)})

        # Concentration diagnostics for deciding whether a smaller card is warranted.
        dog_share = float((b["Odds"] > 0).mean()) if len(b) else 0.0
        plus200_share = float((b["Odds"] >= 200).mean()) if len(b) else 0.0
        seg.append({"Segment_Group":"Concentration","Segment":"All qualifying bets",**summarize_sim_bets(b),"Dog_Share":dog_share,"Plus200_Share":plus200_share})
...
if mode == "Backtest Lab":
    ...
    st.divider()
    st.markdown('<div class="section-kicker">v0.16.2 • MONEYLINE PRICE-BUCKET AUDIT</div>', unsafe_allow_html=True)
    st.caption("Frozen v0.16 lineup champion re-run on the same ≤6h, no-doubleheader 2025 holdout, with favorites/underdogs split into price and edge buckets. This audit does not tune thresholds.")
    st.warning("Purpose: determine whether the apparent edge is broad-based or concentrated in underdogs. This is a diagnostic only; no thresholds are changed based on the 2025 results.")
    st.info("Uses the same frozen v0.16 point-in-time pipeline and betting rules. Zero Odds API credits.")
    v16_upload=st.file_uploader("Upload Moneyline Master CSV for Price-Bucket Audit",type=["csv"],key="lineup_v16_upload")
    if v16_upload is not None:
        try:
            ...
            if st.button("Run Moneyline Price-Bucket Audit",type="primary",key="run_v16_lineup"):
    ...
    if "v16_result" in st.session_state:
        ...
        if st.session_state.pop("v16_just_completed",False): st.success("Price-bucket audit complete — frozen champion splits below.")
        if isinstance(q,pd.DataFrame) and not q.empty:
        ...
        if tt is None or tt.empty: st.error("Lineup test returned no comparison rows.")
            ...
            seg=rr.get("segments",pd.DataFrame()); coef=rr.get("coefficients",pd.DataFrame())
            if isinstance(seg,pd.DataFrame) and not seg.empty:
                with st.expander("v0.16.2 2025 price / side / edge audit",expanded=True):
                    st.dataframe(seg.round({"Hit":3,"Units":2,"ROI":4,"Dog_Share":3,"Plus200_Share":3}),use_container_width=True,hide_index=True)
                    try:
                        pb=seg[seg.get("Segment_Group","")=="Price bucket"].copy()
                        dogs=pb[pb["Segment"].astype(str).str.startswith("Dog")].copy()
                        favs=pb[pb["Segment"].astype(str).str.startswith("Favorite")].copy()
                        dog_bets=int(dogs["Bets"].sum()) if not dogs.empty else 0
                        fav_bets=int(favs["Bets"].sum()) if not favs.empty else 0
                        if dog_bets and not fav_bets:
                            st.warning("The frozen 2025 betting sample contains qualifying underdogs but no qualifying favorites. Treat a dog-heavy live card as a model-concentration signal, not a mandate to bet every dog.")
                        elif dog_bets > max(1, 2*fav_bets):
                            st.info(f"The frozen holdout is dog-heavy ({dog_bets} dog bets vs {fav_bets} favorite bets). Use the bucket table to identify where the historical edge was actually concentrated.")
                    except Exception:
                        pass
            if isinstance(coef,pd.DataFrame) and not coef.empty:
            ...
            if isinstance(seg,pd.DataFrame) and not seg.empty: st.download_button("Download v0.16.2 Price-Bucket Audit CSV",seg.to_csv(index=False).encode("utf-8"),file_name="mlb_v0162_moneyline_price_bucket_audit.csv",mime="text/csv",key="dl_v16_segments")
