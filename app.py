...
def smart_card_label(side, confidence, lineup_confirmed):
    """Selection layer only; does not alter model probabilities or calibration."""
    if side.get("odds") is None or side.get("edge") is None or side.get("ev") is None:
        return "MODEL ONLY"
    odds=float(side["odds"]); edge=float(side["edge"]); ev=float(side["ev"]); legacy=side.get("verdict","PASS")
    # Smart Card sits on TOP of existing production guardrails; it never upgrades a legacy PASS.
    if legacy == "PASS":
        return "PASS"
    # Historical audit: strongest credible zone begins around 10% edge.
    # Large dogs have very small historical samples, so require both a bigger edge and confirmed lineups.
    if odds >= 200:
        if legacy in ("BEST BET","BET") and lineup_confirmed and confidence >= 82 and edge >= .15 and ev >= .12:
            return "OFFICIAL BET"
        if edge >= .10 and ev >= .08:
            return "SECONDARY"
        return "PASS"
    if legacy in ("BEST BET","BET") and edge >= .10 and ev >= .08 and confidence >= (78 if lineup_confirmed else 82):
        return "OFFICIAL BET"
...
def cls(v):
    return {"OFFICIAL BET":"badge-best","SECONDARY":"badge-bet","BEST BET":"badge-best","BET":"badge-bet","LEAN":"badge-lean","PASS":"badge-pass","MODEL ONLY":"badge-lean"}.get(v,"badge-pass")
...
def build_candidates(model_df, games, events):
    ...
    for _,r in model_df.iterrows():
        ...
        selection_rank={"OFFICIAL BET":4,"SECONDARY":3,"MODEL ONLY":2,"PASS":1}
    ...
    order={"OFFICIAL BET":4,"SECONDARY":3,"MODEL ONLY":2,"PASS":1}
...
<div class="hero">
  ...
  <div class="sub">Pitcher 2.0 + offense/platoon + confirmed-lineup upgrade. Smart Card ranks only the strongest priced opportunities; the underlying probabilities are unchanged.</div>
...
if not candidates:
    ...
    if mode == "🎯 Single Game":
        ...
        official=sorted([x for x in candidates if x["market_available"] and x["best"].get("selection")=="OFFICIAL BET"], key=lambda x:x["best"].get("smart_score",-999), reverse=True)[:5]
        secondary=sorted([x for x in candidates if x["market_available"] and x["best"].get("selection")=="SECONDARY"], key=lambda x:x["best"].get("smart_score",-999), reverse=True)
        ...
        st.markdown('<div class="kicker">Smart Official Card</div>',unsafe_allow_html=True)
        if official:
            ...
            st.caption(f"Top {len(official)} official play{'s' if len(official)!=1 else ''} • {dogs} underdog{'s' if dogs!=1 else ''} • {favs} favorite{'s' if favs!=1 else ''}. Official plays generally require 10%+ edge; no side balance is forced.")
        ...
        if not official:
            st.info("No moneyline cleared the Smart Card official threshold right now. Secondary opinions remain visible in the full slate.")
        else:
        ...
        if secondary:
            st.markdown('<div class="kicker">Secondary Watchlist</div>',unsafe_allow_html=True)
            st.caption("7.5–10% edge zone or plays held back by a large-dog/lineup guardrail. These are model opinions, not official bets.")
        ...
        for x in candidates:
            b=x["best"]
            export.append({"Game":x["game"],"Time":x["time"],"Pick":f"{b['team']} ML" if x["market_available"] else f"{b['team']} model lean","Odds":b["odds"],"Book":b["book"],"Smart_Card":b.get("selection"),"Legacy_Grade":b["verdict"],"Calibrated_Prob":b["prob"] if x["market_available"] else None,"Model_Prob":b["raw"],"Edge":b["edge"],"EV":b["ev"],"Fair_ML":b["fair"],"Confidence":x["confidence"],"Lineups_Confirmed":x["lineup_confirmed"],"Market_Available":x["market_available"],"Model_Weight":x["alpha"] if x["market_available"] else None,"Model_Version":MODEL_VERSION})
...
st.markdown("""
<div class="note"><b>Production scope:</b> moneyline only. Starting pitcher + offense/platoon are the core signal. Confirmed batting orders strengthen the live projection; if lineups are not confirmed, the model leans more heavily on the market. Bullpen is neutral because the historical bullpen layer failed to improve the frozen holdout champion. Run lines and totals are intentionally excluded. Smart Card is a separate bet-selection layer: 10%+ edge is the primary official zone, 7.5–10% is secondary, and +200 or longer dogs require a stronger edge plus confirmed lineups. Odds API pulls are manual-only. Single Game can load only the selected matchup; Full Slate has a separate manual pull. Date changes, mode changes, matchup selection and model-only views consume zero odds credits.</div>
""",unsafe_allow_html=True)
